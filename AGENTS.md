...
- `MDEditor/Editor/` — the TextKit 1 stack (`MarkdownTextView` + coordinator), `ColumnLayout` (pure max-width/centering math applied by `MDTextView.applyColumnLayout`; the container never tracks the view width), `StyleEngine` (visual styling; fonts derive from `AppSettings` via `StyleEngine.fontSettings`), `EditingBehavior` and `Autoformat` (headless keystroke/transform engines), `FormatCommands`, `CodeSpellcheck` (strips the checker's `.spellingState` temporary attribute from code ranges; the coordinator re-clears on a debounce after edits), tables, images, paste conversion.
- `MDEditor/Markdown/` — parser, builder, serializer, semantic attribute keys, `MarkdownThumbnailImage` (the Quick Look thumbnail renderer). This folder also compiles into the `MDEditorThumbnail` extension, so it must stay self-contained: never reference the Editor layer from here (that's why the image display-size math lives here as `MDImageDisplay`, with `ImageAttachmentController` delegating to it).
- `MDEditorThumbnail/` — Quick Look thumbnail extension (Finder .md icons show the first screenful of the document): `ThumbnailProvider` reads the requested file (64 KB cap, sandboxed — Quick Look grants access), renders it via `MarkdownThumbnailImage.render` on the main thread, and answers nil on any failure so Finder falls back to the generic icon.
- `MDEditor/Models/` — document/workspace/file-tree models, `AppSettings` (UserDefaults-backed), switch/quit policy and `WorkspaceRootPolicy` (pure, tested headless; the latter decides when an opened file's parent folder replaces the workspace root).
